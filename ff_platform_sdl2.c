#include "ff_platform.h"
#include "ff_touch_sdl2.h"

#include <SDL.h>
#include <stdio.h>
#include <stdlib.h>

static SDL_Window   *ff_win;
static SDL_Renderer *ff_ren;
static SDL_Texture  *ff_tex;
static uint16_t     *ff_pixels;
static ff_surface    ff_back;
static int           ff_w;
static int           ff_h;
static int           ff_keys[FF_KEY_COUNT];
static int           ff_running;
static int           ff_touch;
static int           ff_tap_x;
static int           ff_tap_y;
static int           ff_tap_pending;
static int           ff_back_pending;

int Platform_Init(int w, int h, int scale, const char *title)
{
    if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_TIMER) != 0)
        return 0;
#ifdef __ANDROID__
    ff_touch = 1;
#else
    ff_touch = SDL_getenv("FFRACE_TOUCH") != NULL;
    if (ff_touch)
        SDL_SetHint(SDL_HINT_MOUSE_TOUCH_EVENTS, "1");
#endif
    ff_win = SDL_CreateWindow(title, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
                              w * scale, h * scale, ff_touch ? SDL_WINDOW_RESIZABLE : 0);
    if (!ff_win)
        return 0;
    ff_ren = SDL_CreateRenderer(ff_win, -1,
                                SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
    if (!ff_ren)
        ff_ren = SDL_CreateRenderer(ff_win, -1, SDL_RENDERER_PRESENTVSYNC);
    if (!ff_ren)
        ff_ren = SDL_CreateRenderer(ff_win, -1, 0);
    if (!ff_ren)
        return 0;
    ff_tex = SDL_CreateTexture(ff_ren, SDL_PIXELFORMAT_RGB565,
                               SDL_TEXTUREACCESS_STREAMING, w, h);
    if (!ff_tex)
        return 0;
    /* JumpyBall.exe Game_Init 0x000113bc passes 0 to Gfx_CreateBackBuffer
       0x00021698 when g_viewH 0x0002626c is not 0xf0, so g_clipHRow 0x000269e8
       keeps its .data value 0x140 and Blit_TileH admits y == h. */
    ff_pixels = (uint16_t *)calloc((size_t)w * ((size_t)h + 1u), sizeof(uint16_t));
    if (!ff_pixels)
        return 0;
    ff_w = w;
    ff_h = h;
    ff_back.pixels  = ff_pixels;
    ff_back.bpp     = 0x10;
    ff_back.fmt     = FF_FMT_RGB565;
    ff_back.x_pitch = 1;
    ff_back.y_pitch = w;
    ff_clip_w     = w;
    ff_clip_h     = h;
    ff_clip_h_row = h;
    ff_running = 1;
    return 1;
}
void Platform_Shutdown(void)
{
    Platform_AudioShutdown();
    free(ff_pixels);
    ff_pixels = NULL;
    if (ff_tex)
        SDL_DestroyTexture(ff_tex);
    if (ff_ren)
        SDL_DestroyRenderer(ff_ren);
    if (ff_win)
        SDL_DestroyWindow(ff_win);
    ff_tex = NULL;
    ff_ren = NULL;
    ff_win = NULL;
    SDL_Quit();
}

ff_surface *Platform_BackBuffer(void)
{
    return &ff_back;
}

void Platform_Present(void)
{
    SDL_UpdateTexture(ff_tex, NULL, ff_pixels, ff_w * (int)sizeof(uint16_t));
    if (ff_touch) {
        int ww = 0, wh = 0;

        SDL_GetRendererOutputSize(ff_ren, &ww, &wh);
        Touch_Layout(ww, wh, ff_w, ff_h);
        SDL_SetRenderDrawBlendMode(ff_ren, SDL_BLENDMODE_NONE);
        SDL_SetRenderDrawColor(ff_ren, 0, 0, 0, 0xff);
        SDL_RenderClear(ff_ren);
        SDL_RenderCopy(ff_ren, ff_tex, NULL, Touch_GameRect());
        Touch_Draw(ff_ren);
        SDL_RenderPresent(ff_ren);
        return;
    }
    SDL_RenderClear(ff_ren);
    SDL_RenderCopy(ff_ren, ff_tex, NULL, NULL);
    SDL_RenderPresent(ff_ren);
}

int Platform_TouchActive(void)
{
    return ff_touch;
}

static int ff_keymap[FF_KEY_COUNT] = {
    SDLK_LEFT, SDLK_RIGHT, SDLK_UP, SDLK_DOWN, SDLK_SPACE, SDLK_RETURN
};

#define FF_RAWQ 16

static int ff_rawq[FF_RAWQ];
static int ff_rawq_head;
static int ff_rawq_tail;

static int MapKey(SDL_Keycode code)
{
    int k;

    if ((int)code == FF_KEY_UNBOUND)
        return -1;
    for (k = 0; k < FF_KEY_COUNT; k++) {
        if (ff_keymap[k] == (int)code)
            return k;
    }
    return -1;
}

int Platform_KeyBinding(int key)
{
    if (key < 0 || key >= FF_KEY_COUNT)
        return FF_KEY_UNBOUND;
    return ff_keymap[key];
}

void Platform_SetKeyBinding(int key, int code)
{
    if (key < 0 || key >= FF_KEY_COUNT)
        return;
    ff_keymap[key] = code;
    ff_keys[key]   = 0;
}

int Platform_NextRawKey(void)
{
    int code;

    if (ff_rawq_tail == ff_rawq_head)
        return FF_KEY_UNBOUND;
    code         = ff_rawq[ff_rawq_tail];
    ff_rawq_tail = (ff_rawq_tail + 1) % FF_RAWQ;
    return code;
}

void Platform_FlushRawKeys(void)
{
    ff_rawq_tail = ff_rawq_head;
}

static void NoteTap(int px, int py)
{
    int ww = 0, wh = 0;

    if (ff_touch) {
        const SDL_Rect *r = Touch_GameRect();

        if (r->w < 1 || r->h < 1)
            return;
        if (px < r->x || px >= r->x + r->w || py < r->y || py >= r->y + r->h)
            return;
        ff_tap_x = (px - r->x) * ff_w / r->w;
        ff_tap_y = (py - r->y) * ff_h / r->h;
    } else {
        SDL_GetRendererOutputSize(ff_ren, &ww, &wh);
        if (ww < 1 || wh < 1)
            return;
        ff_tap_x = px * ff_w / ww;
        ff_tap_y = py * ff_h / wh;
    }
    ff_tap_pending = 1;
}

int Platform_NextTap(int *x, int *y)
{
    if (!ff_tap_pending)
        return 0;
    ff_tap_pending = 0;
    *x = ff_tap_x;
    *y = ff_tap_y;
    return 1;
}

int Platform_TakeBack(void)
{
    int b = ff_back_pending;

    ff_back_pending = 0;
    return b;
}

int Platform_PollEvents(void)
{
    SDL_Event ev;
    int k;

    while (SDL_PollEvent(&ev)) {
        switch (ev.type) {
        case SDL_QUIT:
            ff_running = 0;
            break;
        case SDL_FINGERDOWN:
        case SDL_FINGERUP:
        case SDL_FINGERMOTION:
            if (ff_touch) {
                if (ev.type == SDL_FINGERDOWN) {
                    int ww = 0, wh = 0;

                    SDL_GetRendererOutputSize(ff_ren, &ww, &wh);
                    NoteTap((int)(ev.tfinger.x * (float)ww),
                            (int)(ev.tfinger.y * (float)wh));
                }
                Touch_Handle(&ev);
            }
            break;
        case SDL_MOUSEBUTTONDOWN:
            if (!ff_touch && ev.button.button == SDL_BUTTON_LEFT)
                NoteTap(ev.button.x, ev.button.y);
            break;
        case SDL_APP_WILLENTERBACKGROUND:
            Platform_AudioPause(1);
            break;
        case SDL_APP_DIDENTERFOREGROUND:
            Platform_AudioPause(0);
            break;
        case SDL_KEYDOWN:
            if (ev.key.keysym.sym == SDLK_ESCAPE ||
                ev.key.keysym.sym == SDLK_AC_BACK) {
                ff_back_pending = 1;
                break;
            }
            if (ev.key.repeat == 0) {
                int next = (ff_rawq_head + 1) % FF_RAWQ;

                if (next != ff_rawq_tail) {
                    ff_rawq[ff_rawq_head] = (int)ev.key.keysym.sym;
                    ff_rawq_head          = next;
                }
            }
            k = MapKey(ev.key.keysym.sym);
            if (k >= 0)
                ff_keys[k] = 1;
            break;
        case SDL_KEYUP:
            k = MapKey(ev.key.keysym.sym);
            if (k >= 0)
                ff_keys[k] = 0;
            break;
        default:
            break;
        }
    }
    return ff_running;
}

int Platform_KeyDown(int key)
{
    if (key < 0 || key >= FF_KEY_COUNT)
        return 0;
    if (ff_keys[key])
        return 1;
    return ff_touch ? Touch_Down(key) : 0;
}

unsigned Platform_Ticks(void)
{
    return (unsigned)SDL_GetTicks();
}

void Platform_Delay(unsigned ms)
{
    SDL_Delay((Uint32)ms);
}

const char *Platform_BasePath(void)
{
#ifdef __ANDROID__
    return "";
#else
    static char path[512];

    if (path[0] == '\0') {
        char *sdl = SDL_GetBasePath();

        if (sdl != NULL) {
            SDL_strlcpy(path, sdl, sizeof path);
            SDL_free(sdl);
        } else {
            SDL_strlcpy(path, "./", sizeof path);
        }
    }
    return path;
#endif
}

/* SDL_GetBasePath is the only source here, so there is nothing to tell apart;
   the native backend is the one with several. */
const char *Platform_BaseOrigin(void)
{
    return "SDL_GetBasePath";
}

const char *Platform_PrefPath(void)
{
#ifdef __ANDROID__
    static char path[512];

    if (path[0] == '\0') {
        const char *dir = SDL_AndroidGetInternalStoragePath();

        if (dir == NULL)
            return "";
        SDL_snprintf(path, sizeof path, "%s/", dir);
    }
    return path;
#else
    return Platform_BasePath();
#endif
}

int Platform_FileExists(const char *path)
{
    SDL_RWops *rw = SDL_RWFromFile(path, "rb");

    if (rw == NULL)
        return 0;
    SDL_RWclose(rw);
    return 1;
}

unsigned char *Platform_ReadFile(const char *path, long *out_len)
{
    SDL_RWops     *rw;
    Sint64         size;
    unsigned char *buf;

    rw = SDL_RWFromFile(path, "rb");
    if (rw == NULL)
        return NULL;
    size = SDL_RWsize(rw);
    if (size <= 0 || size > 0x2000000) {
        SDL_RWclose(rw);
        return NULL;
    }
    buf = (unsigned char *)malloc((size_t)size);
    if (buf == NULL) {
        SDL_RWclose(rw);
        return NULL;
    }
    if (SDL_RWread(rw, buf, 1, (size_t)size) != (size_t)size) {
        SDL_RWclose(rw);
        free(buf);
        return NULL;
    }
    SDL_RWclose(rw);
    *out_len = (long)size;
    return buf;
}

void Platform_ShowError(const char *title, const char *text)
{
    fprintf(stderr, "%s: %s\n", title, text);
    SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_ERROR, title, text, ff_win);
}

const char *Platform_LastError(void)
{
    return SDL_GetError();
}
